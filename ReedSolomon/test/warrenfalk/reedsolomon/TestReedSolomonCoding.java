...
public class TestReedSolomonCoding {
	...
	public void testCalculateSingleStripe() {
		...
		ByteBuffer[] columns = new ByteBuffer[] {
				...
				ByteBuffer.wrap("xyz".getBytes()),
				ByteBuffer.wrap("012".getBytes()),
		...
		assertEquals("size result of checksum calculation", 3 * checksumSize, size);
		...
		for (long validMask = (1 << wordSize) - 1; validMask > 0; validMask--) {
			...
			for (int i = 0; i < wordSize; i++) {
				if (0 == (validMask & (1 << i)))
					recovered[i] = ByteBuffer.allocate(3);
				else
			...
			assertEquals("size result of recovery calculation for valid mask " + validMask, invalidCount * 3, size);
